...
#                    print(OSN)
    ...
    print("%"*42)
    #____________________________________________________replaced
    len_rep = sum( [len(v) for k,v in rep.items()] )

    mima = "\n".join( sorted( ["%s[<%i;%i>]= %i pcs \t=%s" % 
        ( port, min[port], max[port], len(rep[port]), rep[port] ) for port in min.keys()] ) )

    mimaOther = "\n".join(repOther)
    print("Replaced these: (%i pcs)\n%s\n%s" % (len_rep, mima,mimaOther))
#    print("Replaced these: \nmin:\n%s\nmax:\n%s" % ( str(min), str(max) ) )

    #____________________________________________________not replaced
    len_nrep = sum( [len(v) for k,v in nrep.items()] )

    mima = "\n".join( sorted( ["%s = %i pcs\t=%s" % 
        ( port, len(nrep[port]), nrep[port] ) for port in nrep.keys()] ) )
    print("Not replaced these, but could have (user signal name changeable): (%i pcs)\n%s" 
            % (len_nrep, mima))
