...
#        q.LOAD_settings()
    ...
    def MAKE_intelligence(q):
        ...
        q.ch = {'columns':":", 'tags':"\n", 'ul':"'"}
#book:title:author:note:recomender
        tgs_conf = """book:'title:'author:'note:'recomender:'genre:'language:'done:aws'omness:'wanna
watch:'name:'author:'note:'recomender:'genre:'language:'done:aws'omness:'wanna
"""
        tgs_lists = tgs_conf.split(q.ch['tags'])
        q.tgs = { list.split(q.ch['columns'])[0]:list.split(q.ch['columns'])[1:]
                for list in tgs_lists if list.strip() not in ("") }
#        print(q.tgs)
